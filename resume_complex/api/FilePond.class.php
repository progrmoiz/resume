...
function move_temp_file($file, $path, $name = null)
{
    $_name = is_null($name) ? sanitize_filename($file['name']) : $name;

    move_uploaded_file($file['tmp_name'], $path . DIRECTORY_SEPARATOR . $_name);
}
...
function move_file($file, $path, $name = null)
{
    ...
    if (is_uploaded_file($file['tmp_name'])) {
        return move_temp_file($file, $path, $name);
    }
    return rename($file['tmp_name'], $path . DIRECTORY_SEPARATOR . $_name);
}
