...
function handle_transfer_ids_post($ids)
{
    foreach ($ids as $id) {
        ...
        foreach ($files as $file) {
            ...
            FilePond\move_file($file, UPLOAD_DIR, $targ_file);
            create_userfile(1, $targ_file, $file['name'], $file['length']);
        }
...
function create_userfile($user_id, $targ_file, $file_name, $length)
{
    global $db;

    $sql = "INSERT INTO userFile (user_id, resume, name, length) VALUES (:user_id, :resume, :name, :length)";
    $handle = $db->prepare($sql);
    $handle->bindValue(':user_id', $user_id, PDO::PARAM_INT);
    $handle->bindValue(':resume', $targ_file);
    $handle->bindValue(':name', $file_name);
    $handle->bindValue(':length', $length, PDO::PARAM_INT);
    $handle->execute();
}